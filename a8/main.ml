...
let rec print_stars_bolded str =
  ...
  |[] -> ANSITerminal.(print_string [black;on_white] str); print_string "\n"
  |h::n::t -> if h = 0 
    then let bold = String.sub str 1 (n-1) in 
      ANSITerminal.(print_string [black;on_white;Bold] bold);
      if n = ((String.length str) - 1) then print_string "\n" else 
...
let rec prompt_shuffle () = 
  ...
  | Quit -> (ANSITerminal.(print_string [red] 
             ...
             print_endline "\n"; Pervasives.exit 0)
  | _ -> unknown_user_input (); prompt_shuffle ()
...
let rec prompt_terms () = 
  ...
  | Quit -> (ANSITerminal.(print_string [red] 
                             "\nHope you feel more prepared after studying, you got this!\n");
             print_endline "\n"; Pervasives.exit 0) 
  | _ -> unknown_user_input (); prompt_terms ()
...
let rec choose_mode st =
  ...
  | Quit -> (ANSITerminal.(print_string [red] 
                             "\nHope you feel more prepared after studying, you got this!\n");
             print_endline "\n"; Pervasives.exit 0)
  | Practice -> start_practice_mode {st with mode_deck=st.deck;incorrect=[]; score = 0; current = (Flashcard.first_card st.deck); already_seen=[]} "p"
  | Practice_wrongs -> start_practice_mode {st with mode_deck=st.incorrect; score = 0; current = (Flashcard.first_card st.incorrect); already_seen=[]} "pw"
  | Test_wrongs -> start_test_mode {st with mode_deck=st.incorrect; current = (Flashcard.first_card st.incorrect); already_seen=[]} "tw"
  | Test -> start_test_mode {st with mode_deck=st.deck; score = 0; incorrect=[]; current = (Flashcard.first_card st.deck); already_seen=[]} "t"
  ...
  | _ -> unknown_user_input (); choose_mode st
...
and next_turn_test st mode = 
  ...
  | Quit -> let is_high = st.score > st.high_score in 
    if is_high then (ANSITerminal.(print_string [red] 
          ...
          print_endline "\n"; Pervasives.exit 0)
  | Next -> let got_correct = in_correct_pile st in
    ...
    let _ =  if got_correct then () else if already_seen then () else 
        let correct_ans = if st.prompt = "terms" then card_no_option.back else
          ...
          (ANSITerminal.(print_string [white;on_black] ("\nCorrect answer: " ^ correct_ans)); print_string "\n")
        else 
          ((ANSITerminal.(print_string [black;on_white]
                            ("\nCorrect answer: "))); print_stars_bolded correct_ans) in 
    (let next = State.next st in 
     let next = if got_correct then {next with current_face = next.prompt; already_seen = (remove_option st::st.already_seen)} 
       else {next with current_face = 
                         next.prompt; already_seen = (remove_option st::st.already_seen); incorrect = 
                                                                                            if Flashcard.mem card_no_option next.incorrect 
  ...
  | Text t -> ( let deck = st.mode_deck in 
                ...
                if is_correct_ans then ((ANSITerminal.(print_string [green] ("\nCorrect!\n")));
                  ...
                  let format = if st.prompt = "defs" then [ANSITerminal.white;ANSITerminal.on_black] else 
                      [ANSITerminal.black;ANSITerminal.on_white] in
                  ANSITerminal.(print_string [red] "\nIncorrect");
                  ANSITerminal.(print_string format
                                  ("\nCorrect answer: " ^ correct_ans)); print_string "\n";
  ...
  | _ -> unknown_user_input (); next_turn_test st mode 

and start_test_mode st mode = 
  let rand = prompt_shuffle () in 
  let terms = prompt_terms () in 
  match rand with 
  | true -> let st = (if terms = true then {st with current_face = "terms"; prompt = "terms"} else 
    ...
    (let rand_state = State.randomize st in 
     ...
     | Some card -> let output = if terms then card.front else card.back in
       let format = if terms then [ANSITerminal.white;ANSITerminal.on_black] else 
           [ANSITerminal.black;ANSITerminal.on_white] in
       ANSITerminal.(print_string format
                       ("\n" ^ output)); print_string "\n";
       next_turn_test rand_state mode)
  | false -> let st = (if terms = true then {st with current_face = "terms"; prompt = "terms"} else 
                         {st with current_face = "defs"; prompt = "defs" }) in 
    (let deck = st.mode_deck in 
     match  Flashcard.first_card deck with 
     | None -> ANSITerminal.(print_string [red] "\nThere are no cards in this deck."); choose_mode st
...
and next_turn_practice st mode = 
  ...
  | Quit -> (ANSITerminal.(print_string [red] 
                             "\nHope you feel more prepared after studying, you got this!\n");
             print_endline "\n"; Pervasives.exit 0)
  | Flip -> (if st.current_face = "terms" then (print_defs st; next_turn_practice ({st with current_face ="defs"}) mode )
  ...
  | Unstar -> let new_starred = State.unstar_card st in 
    ...
    (next_turn_practice ({st with starred = new_starred}) mode)
  | _ -> unknown_user_input (); next_turn_practice st mode
...
let rec start_study deck =
  ...
  | Quit -> (ANSITerminal.(print_string [red] 
                             "\nHope you feel more prepared after studying, you got this!\n");
             print_endline "\n"; Pervasives.exit 0)
  | Practice -> let st = State.init_state deck in start_practice_mode st "p"
  | Test -> let st = State.init_state deck in start_test_mode st "t"
